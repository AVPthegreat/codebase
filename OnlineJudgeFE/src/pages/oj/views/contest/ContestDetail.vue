      ...
      <div class="flex-container" v-if="route_name === 'contest-details'">
        <template>
          <div id="contest-desc">
            <Panel :padding="20" shadow>
              ...
              <div slot="extra" class="countdown-actions">
                ...
                <Button
                  ...
                  ghost
                  class="stop-btn"
                  @click="handleStopContest"
                  >Stop</Button
                >
              </div>
              <div v-if="contestStarted" class="proctoring-banner">
                <Icon type="alert-circled" /> Proctoring active. Fullscreen
                required. Exits: {{ fullscreenExitCount }}/5
              </div>
              <div
                v-if="!contestStarted"
              ...
              <div
                v-if="passwordFormVisible && !contestStarted"
            ...
            ></Table>
            <Alert
              v-if="contestStarted && !isFullscreen"
              type="warning"
              show-icon
              class="proctoring-warning"
            >
              You exited full screen during contest. Please return to full
              screen. Warning {{ fullscreenExitCount }}/5
            </Alert>
          </div>
        </template>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      overviewColumns: [
        { title: "ID", key: "_id", width: 80, align: "center" },
          ...
          render: (h, params) => {
            ...
            if (params.row.attempts > 0) {
              if (r === 0) {
                ...
                r === 8 ||
                (params.row.passed_cases > 0 &&
                  params.row.passed_cases < params.row.total_cases)
              ) {
                text = "PC";
                color = "warning";
              } else {
  ...
  methods: {
    ...
    handleStartContest() {
      this.$Modal.confirm({
        title: "Start Contest",
        content:
          "You will not be able to access other parts until you submit the contest. Are you sure to start?",
        onOk: () => {
          // Immediately set started state for instant UI update
          this.$store.commit(types.CONTEST_SET_STARTED, {
            started: true,
            resetCount: true,
            contestID: this.contestID,
          });
          api
            .contestStart(this.contestID)
            .then((res) => {
    ...
    handleStopContest() {
      const id =
        this.attemptId ||
        (this.$store.state.contest.attempt &&
          this.$store.state.contest.attempt.id);
      if (!id) {
      ...
      this.$Modal.confirm({
        ...
        content:
          "This will submit all your work and end the contest. Are you sure?",
        onOk: () => {
          // Submit all localStorage data to backend
          this.$store
            .dispatch("submitContestFromLocal", this.contestID)
            .then(() => {
              ...
              onOk: () => {
                ...
                if (el.requestFullscreen) {
                  el.requestFullscreen().catch(() => {
                    this.$Message.error(
                      "Failed to enter fullscreen. Please press F11 or use browser fullscreen."
                    );
                  });
                } else if (el.webkitRequestFullscreen) {
                  el.webkitRequestFullscreen();
  ...
  computed: {
    ...
    contestStarted() {
      return this.$store.state.contest.started;
    },
    fullscreenExitCount() {
      return this.$store.state.contest.fullscreenExitCount;
    },
    isFullscreen() {
      return !!(
        document.fullscreenElement ||
...
.countdown-actions {
  ...
  align-items: center;
  gap: 10px;
  flex-wrap: wrap; /* Allow wrapping to prevent cropping */
...
/* Note: These selectors assume a parent class or just force styles if the theme is dark */
/* Since we don't have a reliable dark mode class on body yet, we'll use a specific override that works when the background is dark */

/* Force Panel Title to be white in dark contexts or just generally readable */
/deep/ .ivu-card-head p,
/deep/ .panel-title {
  color: inherit; /* Inherit from parent, which should be handled by theme */
}

/* Specific fix for "NexusCode Pilot Contest" title if it's not picking up theme */
/* We can force it to be white if we are sure we are in dark mode, but safer to use a high-contrast color or variable */
</style>

<!-- Global styles for Modal to fix "Start Contest" popup in dark mode -->
<style>
/* This will affect all Modals, which is usually desired for consistency in dark mode */
/* Assuming the user has a dark theme class on body or similar, but if not, we target the specific modal class if possible */

/* If the app uses a dark theme class on body, e.g., .theme-dark */
.theme-dark .ivu-modal-confirm-head-title {
  color: #fff !important;
}

/* Fallback: If we want to force it for this specific context, we might need a more specific selector or just apply it generally if the user is always in dark mode */
/* Given the user request "in dark mode", I'll assume there is a mechanism. 
     However, to be safe and fix the user's immediate issue: */
...
/* Also force it if the parent container has dark background (common in these apps) */
.ivu-modal-content {
  /* Ensure modal content background is dark if we are changing text to white, otherwise it might be invisible on white */
  /* We won't touch background here to avoid breaking light mode, but we will ensure title is visible */
}

/* Fix for the specific element requested: <span class="modal-title-dark">Start Contest</span> */
/* It seems the user might have added this class or wants us to target it? 
     The user said: "Start Contest Popup, in this the heading is still black in dark mode ... this element"
     I will add a rule for .ivu-modal-confirm-head-title to be white when inside a dark theme context.
  */

.ivu-modal-confirm-head-title {
  color: #ececec; /* Default to a lighter grey which works on both usually, or strictly white for dark mode */
}

/* Fix for Contest Title "NexusCode Pilot Contest" */
#contest-desc .ivu-card-head p {
  color: #ececec !important; /* Force light color for contest title */
}

/* Revert for light mode if needed - assuming the app is primarily dark or user wants dark mode fixes */
</style>
